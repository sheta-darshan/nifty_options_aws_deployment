...
def get_contract_price_on_day(cache_dir, strike, opt_type, exp_idx, expiry_date, date_str):
    # Match nifty_{strike}_{opt_type}_exp{exp_idx}_expiry{expiry_date}_{date_str}.csv
    ...
    if os.path.exists(file_path):
        try:
            ...
            if not df_c.empty:
                # Filter for 15:20 candle if available, else last candle
...
def run_parameter_backtest(entry_weekday=2, early_exit_target_pts=None):
    ...
    for i, curr_date in enumerate(target_dates):
        ...
        # Check active positions for early exit or expiry at 15:20 on every day
        ...
        for stance in ["CALL", "PUT"]:
            ...
            # 1. Check for Early Exit Target if enabled
            if early_exit_target_pts is not None:
                # Verify if all 3 legs are active
                if port["monthly_long"] is not None and port["weekly_short1"] is not None and port["weekly_short2"] is not None:
                    # Fetch current price of all legs on this day
                    ...
                    if l_val is not None and s1_val is not None and s2_val is not None:
                        long_pnl_pts = 3 * (l_val - port["monthly_long"]["entry_px"])
                        short1_pnl_pts = 1 * (port["weekly_short1"]["entry_px"] - s1_val)
                        short2_pnl_pts = 1 * (port["weekly_short2"]["entry_px"] - s2_val)
                        total_open_pnl_pts = long_pnl_pts + short1_pnl_pts + short2_pnl_pts
                        
                        if total_open_pnl_pts >= early_exit_target_pts:
                            # Target reached! Trigger early exit for all 3 legs
        ...
        # Entry check: entry_weekday at 09:20 AM
        if curr_weekday == entry_weekday:
...
if __name__ == "__main__":
    ...
    # 1. Sweep over entry weekdays: Wednesday (2), Thursday (3), Friday (4)
    # 2. Sweep over early exit targets (in points): None (hold to expiry), 100, 150, 200
    weekday_names = {2: "Wednesday", 3: "Thursday", 4: "Friday"}
    
    print("="*100)
    print("            STRATEGY 20 PARAMETER OPTIMIZATION SWEEP")
    print("="*100)
    
    for wday in [2, 3, 4]:
        for exit_target in [None, 100, 150, 200]:
            print(f"[RUNNING] Entry: {weekday_names[wday]} | Early Exit Target: {exit_target} pts...")
            df = run_parameter_backtest(entry_weekday=wday, early_exit_target_pts=exit_target)
            if not df.empty:
                num_trades = len(df)
                ...
                results.append({
                    "Entry_Day": weekday_names[wday],
                    "Early_Exit_Target": f"{exit_target} pts" if exit_target else "Hold to Expiry",
    ...
    print("\n" + "="*100)
    print("                         OPTIMIZATION SWEEP RANKINGS")
    print("="*100)
    print(df_res.to_string(index=True))
    print("="*100)
    df_res.to_csv("strategy20_optimization_results.csv", index=False)
