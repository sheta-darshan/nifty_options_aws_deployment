...
def run_180day_exact_premium_matched_backtest():
    ...
    print(f"[INFO] Premium-Matched Backtest Window: {start_str} to {end_str} ({len(target_dates)} Trading Days / Last 180 Days).")
    ...
    while i < len(target_dates) - 5:
        ...
        if (curr_date.weekday() in [1, 3]) and current_pos is None:
            ...
            # Scan all cached 1-minute option contract files for entry_ts
            ...
            # Match Target Premiums EXACTLY like choose_calendar_spread_v2 in Live Bot:
        ...
        if current_pos is not None:
            ...
            if days_held >= 5 or i == len(target_dates) - 1:
                ...
                if current_pos['opt_type'] == 'PE':
                    short1_exit_px = max(0.0, current_pos['strike_short1'] - exit_spot)
                    short2_exit_px = max(0.0, current_pos['strike_short2'] - exit_spot)
    ...
    print("\n" + "="*95)
    print("  EXACT LIVE BOT PREMIUM-MATCHED BACKTEST (180 DAYS / 24 CYCLES): PROF. CHIRAG JAIN 3:1:1")
    print("="*95)
    ...
    out_csv = "research_and_development/strategy20_exact_live_premium_matched_log.csv"
    trades_df.to_csv(out_csv, index=False)
    print(f"\n[SUCCESS] Exact live premium-matched trade log saved to: {out_csv}")

if __name__ == "__main__":
    run_180day_exact_premium_matched_backtest()
