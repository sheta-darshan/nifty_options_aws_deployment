...
#### D. Resampling & Avoiding Lookahead Bias (CRITICAL)
* **The Problem**: When resampling 1-minute data to higher timeframes (e.g., 5-minute or 15-minute) in a backtest, the candle label (usually the left edge, like `09:30:00`) represents the period from `09:30:00` to `09:45:00`. If you query the resampled DataFrame at index `09:30:00` to make an entry decision at `09:30:00`, you are using future data (up to `09:44:59`). This introduces **lookahead bias**, causing highly profitable but unrealistic backtest results that fail in live trading.
* **The Prevention Rules**:
  1. **Strict Inequality Filtering**: When slicing a resampled historical dataframe at a specific entry time `entry_dt` (like `09:30:00`), always use strict inequality `<` instead of `<=` to ensure you only read candles that are 100% complete and closed:
     ```python
     valid_15 = df_15[df_15.index < entry_dt]  # Returns candles up to 09:15 (ending at 09:30)
     ```
  2. **Indicator Shifting**: When merging resampled higher-timeframe indicators back into the 1-minute dataframe, always shift the resampled indicators by 1 candle to represent their completed state:
     ```python
     df_15['trend_15'] = df_15['trend_15'].shift(1).fillna(1)
     df['trend_15'] = df_15['trend_15'].reindex(df.index, method='ffill').fillna(1)
     ```
     This ensures that the live bot's calculation (which drops incomplete candles) matches the backtester's calculation exactly.
...
| **Strategy_15** | [Strategy15](file:///g:/100%20Days%20of%20code/boxdata/Live%20trading/nifty_options_aws_deployment/strategies/strategy_15.py) | Opening Pattern Scalp | 1-Minute | Evaluates opening 15-minute price action. Enters momentum option trades at 09:45 AM based on opening high/low breakouts. |
| **Strategy_16** | [Strategy16](file:///g:/100%20Days%20of%20code/boxdata/Live%20trading/nifty_options_aws_deployment/strategies/strategy_16.py) | VWAP Mean Reversion | 1-Minute | Fades extreme distance from VWAP using Bollinger Band outer band touches and RSI oversold/overbought confirmations. |
| **Strategy_17** | [Strategy17](file:///g:/100%20Days%20of%20code/boxdata/Live%20trading/nifty_options_aws_deployment/strategies/strategy_17.py) | Multi-Timeframe Supertrend | 1-Min / 5-Min / 15-Min | Alignment strategy requiring 1-min, 5-min, and 15-min Supertrends to point in the same direction before firing entries. |
| **Strategy_18** | [Strategy18](file:///g:/100%20Days%20of%20code/boxdata/Live%20trading/nifty_options_aws_deployment/strategies/strategy_18.py) | 50 SMA Breakout | 5-Minute | Enters directional breakout trades on 50 SMA slope confirmation and volume expansion. |
| **Strategy_19** | [Strategy19](file:///g:/100%20Days%20of%20code/boxdata/Live%20trading/nifty_options_aws_deployment/strategies/strategy_19.py) | Advanced StochRSI Pullback | 5-Minute | Captures pullbacks in strong trend regimes defined by EMA 200, entering on StochRSI zero-line crossovers. |
...
| **Strategy_24** | [Strategy24](file:///g:/100%20Days%20of%20code/boxdata/Live%20trading/nifty_options_aws_deployment/strategies/strategy_24.py) | Coiled Pre-Breakout / Pre-Breakdown Momentum | 1-Min / Daily | **Institutional Coiled Momentum Smart Hybrid Engine**. Bidirectional Long Breakout / Short Breakdown over Day T High/Low with 5m confirmation. Filters: VCP wave shrinkage $\le 0.52$, Smart Money Footprint (CAR $\ge 18\%$, CLV, UVR), Anti-Gap Exhaustion ($< 0.40\times$ ATR), Anti-Rejection Wick ($< 45\%$). Smart Hybrid routes F&O stocks to official options and non-F&O to 5x MIS cash equities. 3-Yr Backtest: 59.9% WR, 2.26 PF, +₹75.2k (1x) / +₹3.75L (5x MIS). |
