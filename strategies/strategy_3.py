...
class Strategy3(BaseStrategy):
    ...
    def generate_signals(self, df_spot: pd.DataFrame) -> pd.DataFrame:
        ...
        df_5min['TM_EMA_LONG'] = ta.ema(df_5min['close'], length=self.params["TM_EMA_LONG"])
        df_5min['TM_EMA_SHORT'] = ta.ema(df_5min['close'], length=self.params["TM_EMA_SHORT"])
        df_5min['TM_EMA_BASE'] = ta.ema(df_5min['close'], length=self.params["TM_EMA_BASE"])
        
        df_5min['TM_EMA_SHORT_Slope'] = df_5min['TM_EMA_SHORT'].diff()
        df_5min['TM_EMA_BASE_Slope'] = df_5min['TM_EMA_BASE'].diff()
        
        adx_tm = ta.adx(df_5min['high'], df_5min['low'], df_5min['close'], length=14)
        if adx_tm is not None:
            df_5min['TM_ADX'] = adx_tm['ADX_14']
        ...
        st_tm = ta.supertrend(df_5min['high'], df_5min['low'], df_5min['close'], 
                              length=self.params["TM_ST_LEN"], multiplier=self.params["TM_ST_MUL"])
        ...
        df['TM_Signal'] = 0
        df['S4_Signal'] = 0
        tm_buy = (df['TM_Long_Trend'] == True) & (df['close'] > df['TM_Sig_High'])
        tm_sell = (df['TM_Short_Trend'] == True) & (df['close'] < df['TM_Sig_Low'])
