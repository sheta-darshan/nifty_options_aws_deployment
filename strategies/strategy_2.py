...
class Strategy2(BaseStrategy):
    ...
    def generate_signals(self, df_spot: pd.DataFrame) -> pd.DataFrame:
        ...
        df_15min['ST15_Dir'] = fast_supertrend_dir(df_15min['high'], df_15min['low'], df_15min['close'], 
                                                   self.params["S2_ST15_LEN"], self.params["S2_ST15_MUL"])
        df_15min['ST15_Dir'] = df_15min['ST15_Dir'].shift(1)
        
        # Map back to 5-min
        df_5min = df_5min.join(df_15min[['ST15_Dir']], how='left')
        df_5min['ST15_Dir'] = df_5min['ST15_Dir'].ffill()
        
        # 5-min Supertrend for Strategy 2
        df_5min['ST5_S2_Dir'] = fast_supertrend_dir(df_5min['high'], df_5min['low'], df_5min['close'], 
                                                    self.params["S2_ST15_LEN"], self.params["S2_ST5_MUL"])
        
        df_5min['S2_Buy'] = (df_5min['ST15_Dir'] == 1) & (df_5min['ST5_S2_Dir'] == 1) & \
                            (df_5min['Stoch_K'] < self.params["S2_RSI_OVERSOLD"]) & bullish_cross
        df_5min['S2_Sell'] = (df_5min['ST15_Dir'] == -1) & (df_5min['ST5_S2_Dir'] == -1) & \
                             (df_5min['Stoch_K'] > self.params["S2_RSI_OVERBOUGHT"]) & bearish_cross
                             
        # 3. Shift 5-min indicators to prevent lookahead
        ...
        df.loc[df['S2_Buy'], 'Strat2_Signal'] = 1
        df.loc[df['S2_Sell'], 'Strat2_Signal'] = -1
        
        df['Signal_Source'] = "None"
        df.loc[(df['Strat2_Signal'] != 0), 'Signal_Source'] = "Strategy 2 (Pullback)"
